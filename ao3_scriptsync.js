...
(function() {
        ...
        static inject() {
            const styles = `
                <style type="text/css">
                    ...
                    /* Sync widget styles - FicTracker design */
                    @keyframes ss-spin {
                        to { transform: rotate(360deg); }
                    }
                    
                    /* Status display with loading states - FicTracker style */
                    ...
                    /* ScriptSync internal data styles */
                    .ss-internal-row {
                        opacity: 0.5;
                        background: #f9f9f9;
                    }
    ...
    class StorageManager {
        ...
        getItem(key, defaultValue = null) {
            ...
            try {
                ...
                console.error('[ScriptSync] Error parsing stored value for key:', key, e);
                return defaultValue;
            }
        }
    ...
    class RemoteSyncManager {
        ...
        updateWidget(state = 'normal') {
            if (!this.settings.syncWidgetEnabled || !this.settings.syncInitialized) return;
            ...
            if (!this.widget) {
                ...
                if (!document.getElementById('ss-spin')) {
                    document.head.insertAdjacentHTML('beforeend', '<style id="ss-spin">@keyframes ss-spin{to{transform:rotate(360deg)}}</style>');
                }

                // Click handler
                this.widget.onclick = () => this.isOnline && !this.isSyncing && this.performSync();

                // Hover effects
                this.widget.onmouseenter = () => !this.isSyncing && Object.assign(this.widget.style, {
                    opacity: '1',
                    background: '#f8f9fa',
                    borderColor: '#0066cc',
            ...
            const states = {
                ...
                syncing: ['#e3f2fd', '#2196f3', '#1976d2', 'ss-spin 1s linear infinite', 'default', 'Syncing...'],
        ...
        addPendingChange(key, value) {
            ...
            pendingChanges.operations.push({ key, value });
        ...
        async performSync() {
            if (!this.isOnline) {
                console.log('[ScriptSync] Offline, skipping sync');
                return;
            }

            if (this.isSyncing) {
                console.log('[ScriptSync] Already syncing, skipping');
                return;
            }

            if (!this.settings.sheetUrl || !this.settings.syncInitialized) {
                console.log('[ScriptSync] Not initialized, skipping sync');
                return;
            }
            ...
            try {
                ...
                if (response.data && response.data.success) {
                    ...
                    console.log('[ScriptSync] Sync successful');
                } else {
                    throw new Error('Sync response indicates failure');
                }
            } catch (error) {
                console.error('[ScriptSync] Sync error:', error);
        ...
        sendSyncRequest(data) {
            return new Promise((resolve, reject) => {
                console.log('[ScriptSync] Sending request:', data.action, 'to', this.settings.sheetUrl);
                ...
                GM_xmlhttpRequest({
                    ...
                    onload: (response) => {
                        console.log('[ScriptSync] Response status:', response.status, 'Response text:', response.responseText.substring(0, 200));
                        try {
                            ...
                            console.error('[ScriptSync] Failed to parse response:', e);
                            reject(new Error('Invalid JSON response: ' + response.responseText.substring(0, 100)));
                        }
                    },
                    onerror: (error) => {
                        console.error('[ScriptSync] Request error:', error);
                        reject(new Error('Network error: ' + (error.error || error)));
                    },
                    ontimeout: () => {
                        console.error('[ScriptSync] Request timeout after 30 seconds');
        ...
        updateLocalStorage(serverData) {
            ...
            // Update local storage for each synced key
            ...
            console.log('[ScriptSync] Local storage updated with server data');
        }
    ...
    class UIManager {
        ...
        injectMenu() {
            ...
            if (dropdown) {
                ins(dropdown, 'beforeend', `
                    <li>
                        <a href="#" id="ss-open-storage">ScriptSync</a>
                    </li>
                `);
        ...
        showStorageView() {
            ...
            for (let i = 0; i < localStorage.length; i++) {
                ...
                storageList.push([key, localStorage.getItem(key), this.getScriptSource(key)]);
            }
            ...
            if (storageList.length > 0) {
                ...
                storageList.forEach(([key, value, source]) => {
                    ...
                    const warningIcon = isFicTrackerKey ? '⚠️ ' : (isScriptSyncKey ? '🔧 ' : '');
                    const toggleTitle = isFicTrackerKey ? ' title="FicTracker data should use FicTracker\'s built-in sync for best results"' : (isScriptSyncKey ? ' title="ScriptSync internal data - not syncable"' : '');
                ...
                html += `<h2>ScriptSync</h2><p>No localStorage data found for AO3 userscripts.</p>`;
            }
        ...
        getSettingsSectionsHTML(settings) {
            ...
            const initDisabled = !settings.sheetUrl || isInitialized ? 'disabled' : '';
            
            // Format last sync time
            const lastSyncTime = settings.lastSync ? new Date(settings.lastSync).toLocaleString() : 'Never';
            ...
            if (!settings.ftWarningDismissed) {
                ...
                <div class="required notice" id="ss-ft-notice">
                    ...
                    use FicTracker's Google Sheets sync instead of ScriptSync. FicTracker's sync includes conflict resolution and 
                    data-aware merging that ScriptSync cannot provide.
                </div>`;
            }
            ...
            if (!isInitialized) {
                html += `
                            ...
                            <input type="submit" value="Initialize Google Sheet" id="ss-initialize" ${initDisabled}>
                ...
                html += `
                            ...
                            <input type="submit" value="Save Settings" id="ss-save-settings">
        ...
        setupEventListeners() {
            ...
            if (toggleUnsyncedBtn) {
                toggleUnsyncedBtn.addEventListener('click', () => {
                    ...
                    const isHiding = !unsyncedRows[0]?.classList.contains('ss-row-hidden');
            ...
            // Save settings
            const saveSettings = q('#ss-save-settings');
            if (saveSettings) {
        ...
        handleSyncToggle(event) {
            ...
            if (this.storage.getSettings().syncInitialized) {
                ...
                const newlyEnabledKeys = selectedKeys.filter(key => !oldSelectedKeys.includes(key));
                ...
                if (newlyEnabledKeys.length > 0) {
                    newlyEnabledKeys.forEach(key => {
                        ...
                        if (value !== null) {
                            ...
                            console.log('[ScriptSync] Added pending change for newly enabled key:', key);
                        }
                    });
                }
        ...
        async updateServerEnabledKeys(keys) {
            try {
                ...
                console.log('[ScriptSync] Server enabled keys updated');
            } catch (error) {
                console.error('[ScriptSync] Failed to update server enabled keys:', error);
            }
        }
        ...
        importData(event) {
            ...
            reader.onload = (e) => {
                try {
                    ...
                    if (!Array.isArray(data) || data.length < 1 || !data.every(item => Array.isArray(item) && item.length === 2)) {
                    ...
                    data.forEach(([key, value]) => {
                        ...
                        if (settings.syncInitialized && settings.selectedKeys.includes(key)) {
                            this.syncManager.addPendingChange(key, value);
                            syncedKeysCount++;
                            console.log('[ScriptSync] Queued imported key for sync:', key);
                        }
                    });
                    ...
                    if (syncedKeysCount > 0) {
                        ...
                        setTimeout(() => {
                            console.log('[ScriptSync] Auto-triggering sync after import');
        ...
        async testConnection() {
            ...
            if (!url.startsWith('https://script.google.com/macros/s/')) {
                this.showStatus('Invalid URL format. Must start with https://script.google.com/macros/s/', 'error');
                return;
            }
            ...
            try {
                ...
                if (response.status === 'success') {
                    this.showStatus('Connection successful! You can now initialize the sync.', 'success');
                    ...
                    this.showStatus('Connection test failed: ' + (response.error?.message || 'Unknown error'), 'error');
                    if (testBtn) {
        ...
        async initializeSync() {
            ...
            console.log('[ScriptSync] Starting initialization');
            ...
            try {
                ...
                try {
                    ...
                    const serverInitialized = serverProbeResponse.data?.initialized || false;
                    serverEnabledKeys = serverProbeResponse.data?.enabled_keys || [];
                    console.log('[ScriptSync] Server probe: initialized=' + serverInitialized + ', keys=' + serverEnabledKeys.length, serverEnabledKeys);
                    ...
                    if (serverInitialized && serverEnabledKeys.length > 0) {
                        ...
                        console.log('[ScriptSync] Server already configured with', serverEnabledKeys.length, 'keys. Adopting server configuration.');
                        this.showStatus(`Found existing configuration with ${serverEnabledKeys.length} key(s). Downloading...`, 'loading');
                        
                        // Download all server data
                        Object.keys(serverData).forEach(key => {
                            localStorage.setItem(key, serverData[key]);
                            console.log('[ScriptSync] Downloaded key:', key);
                        });
                        ...
                        this.showStatus(`Initialized! Downloaded ${serverEnabledKeys.length} key(s) from server. Auto-sync enabled.`, 'success');
                        console.log('[ScriptSync] Initialization successful (adopted server configuration)');
                ...
                if (selectedKeys.length === 0) {
                    ...
                    console.error('[ScriptSync]', errorMsg);
                ...
                console.log('[ScriptSync] Server is empty, initializing with', selectedKeys.length, 'local keys');
                ...
                console.log('[ScriptSync] Sending initialize request with', Object.keys(localData).length, 'keys');
                ...
                console.log('[ScriptSync] Initialize response:', initResponse);
                ...
                if (initResponse.status === 'success') {
                    ...
                    this.showStatus('Initialized! Your local data is now syncing. Auto-sync enabled.', 'success');
                    console.log('[ScriptSync] Initialization successful');
                    ...
                    const errorMsg = 'Initialization failed: ' + (initResponse.error?.message || 'Unknown error');
                    console.error('[ScriptSync]', errorMsg);
                    this.showStatus(errorMsg, 'error');
                    if (initBtn) {
                ...
                console.error('[ScriptSync]', errorMsg, error);
                this.showStatus(errorMsg, 'error');
                if (initBtn) {
                    initBtn.value = '✗ Failed';
                    setTimeout(() => {
        ...
        resetSync() {
            if (!confirm('This will reset all sync settings. You will need to re-initialize. Continue?')) {
                return;
            }
        ...
        async clearServerData() {
            if (!confirm('This will clear all sync data from the Google Sheet, allowing you to re-initialize.\n\nYour local localStorage will not be affected.\n\nContinue?')) {
                return;
            }
            ...
            try {
                ...
                if (response.status === 'success') {
                    this.showStatus('Server data cleared. You can now initialize with your selected keys.', 'success');
                    const initBtn = q('#ss-initialize');
                    if (initBtn) {
                        initBtn.disabled = false;
                    }
                    ...
                    this.showStatus('Failed to clear server data: ' + (response.error?.message || 'Unknown error'), 'error');
                    if (clearBtn) {
                ...
                this.showStatus('Failed to clear server data: ' + error.message, 'error');
                if (clearBtn) {
                    clearBtn.value = '✗ Failed';
                    setTimeout(() => {
                        clearBtn.value = originalText;
        ...
        saveSettings() {
            const syncInterval = parseInt(q('#ss-sync-interval').value);
            const widgetOpacity = parseFloat(q('#ss-widget-opacity').value);
            const saveBtn = q('#ss-save-settings');

            if (syncInterval < 60) {
                alert('Sync interval must be at least 60 seconds.');
                return;
            }

            const originalText = saveBtn ? saveBtn.value : '';
            if (saveBtn) {
                saveBtn.value = 'Saving...';
                saveBtn.disabled = true;
            }

            this.storage.saveSetting('syncInterval', syncInterval);
            this.storage.saveSetting('syncWidgetOpacity', widgetOpacity);

            this.syncManager.settings.syncInterval = syncInterval;
            this.syncManager.settings.syncWidgetOpacity = widgetOpacity;
            ...
            if (this.syncManager.widget) {
                this.syncManager.widget.style.opacity = widgetOpacity;
            }
        ...
        startCountdownUpdater() {
            ...
            this.countdownInterval = setInterval(() => {
                ...
                if (lastSyncSpan && settings.lastSync) {
                    lastSyncSpan.textContent = new Date(settings.lastSync).toLocaleString();
                }
        ...
        getScriptSource(key) {
            ...
            const scriptMap = {
                ...
                'aia_refdate': '<a href="https://greasyfork.org/en/scripts/475525">AO3: Mark Co- and Solo-Wrangled Fandoms</a>',
                'aia_ref': '<a href="https://greasyfork.org/en/scripts/475525">AO3: Mark Co- and Solo-Wrangled Fandoms</a>',
                'floatcmt': '<a href="https://greasyfork.org/en/scripts/489335">AO3: Sticky Comment Box</a>',
                'glossary': '<a href="https://greasyfork.org/en/scripts/450347">AO3: Glossary Definition Previews</a>',
                'agecheck_new': '<a href="https://greasyfork.org/en/scripts/444335">AO3: [Wrangling] Highlight Bins with Overdue Tags</a>',
                'agecheck_old': '<a href="https://greasyfork.org/en/scripts/444335">AO3: [Wrangling] Highlight Bins with Overdue Tags</a>',
                'cmtfmtcustom': '<a href="https://greasyfork.org/en/scripts/484002">AO3: Comment Formatting and Preview</a>',
                'cmtfmtstandard': '<a href="https://greasyfork.org/en/scripts/484002">AO3: Comment Formatting and Preview</a>',
                'iconify0': 'Iconify (icon library)',
                'iconify-count': 'Iconify (icon library)',
                'iconify-version': 'Iconify (icon library)',
                'kbdpages': '<a href="https://greasyfork.org/en/scripts/451524">AO3: [Wrangling] Keyboard Shortcuts</a>',
                'kbdshortcuts': '<a href="https://greasyfork.org/en/scripts/451524">AO3: [Wrangling] Keyboard Shortcuts</a>',
                'smallertagsearch': '<a href="https://greasyfork.org/en/scripts/443886">AO3: [Wrangling] Smaller Tag Search</a>',
                'unread_inbox_count': '<a href="https://greasyfork.org/en/scripts/474892">AO3: Badge for Unread Inbox Messages</a>',
                'unread_inbox_date': '<a href="https://greasyfork.org/en/scripts/474892">AO3: Badge for Unread Inbox Messages</a>',
                'unread_inbox_conf': '<a href="https://greasyfork.org/en/scripts/474892">AO3: Badge for Unread Inbox Messages</a>',
                'script-replaceYN': '<a href="https://greasyfork.org/en/scripts/477499">AO3: Replace Y/N in works with your name</a>',
                'script-replaceYN-on': '<a href="https://greasyfork.org/en/scripts/477499">AO3: Replace Y/N in works with your name</a>',
                'tags_saved_date_map': '<a href="https://greasyfork.org/en/scripts/438063">AO3: [Wrangling] UW Tag Snooze Buttons</a>',
                'kudoshistory_kudosed': '<a href="https://greasyfork.org/en/scripts/5835">AO3: Kudosed and seen history</a>',
                'kudoshistory_checked': '<a href="https://greasyfork.org/en/scripts/5835">AO3: Kudosed and seen history</a>',
                'kudoshistory_seen': '<a href="https://greasyfork.org/en/scripts/5835">AO3: Kudosed and seen history</a>',
                'kudoshistory_bookmarked': '<a href="https://greasyfork.org/en/scripts/5835">AO3: Kudosed and seen history</a>',
                'kudoshistory_skipped': '<a href="https://greasyfork.org/en/scripts/5835">AO3: Kudosed and seen history</a>',
                'ao3tracking_list': '<a href="https://greasyfork.org/en/scripts/8382">AO3: Tracking</a>',
                'ao3tracking_lastcheck': '<a href="https://greasyfork.org/en/scripts/8382">AO3: Tracking</a>',
                'wrangleActionButtons': '<a href="https://greasyfork.org/en/scripts/501991">AO3: [Wrangling] Action Buttons Everywhere</a>',
                'wrangleShortcuts_act': '<a href="https://greasyfork.org/en/scripts/507705">AO3: [Wrangling] Keyboard Shortcuts</a>',
                'wrangleShortcuts_tag': '<a href="https://greasyfork.org/en/scripts/507705">AO3: [Wrangling] Keyboard Shortcuts</a>',
                'rainbowTables': '<a href="https://greasyfork.org/en/scripts/445805">AO3: [Wrangling] Rainbow Tables</a>',
                'wrangleResources': '<a href="https://greasyfork.org/en/scripts/511102">AO3: [Wrangling] Fandom Resources Quicklinks</a>',
                'ao3_chapter_shortcuts_config': '<a href="https://greasyfork.org/en/scripts/549571">AO3: Chapter Shortcuts</a>',
                'ao3_wizard_config': '<a href="https://greasyfork.org/en/scripts/550537">AO3: Site Wizard</a>',
                'ao3_reading_quality_config': '<a href="https://greasyfork.org/en/scripts/549777">AO3: Reading Time & Quality Score</a>',
                'ao3_advanced_blocker_config': '<a href="https://greasyfork.org/en/scripts/549942">AO3: Advanced Blocker</a>',
                'FT_finished': '<a href="https://greasyfork.org/en/scripts/513435">AO3 FicTracker</a>',
                'FT_favorites': '<a href="https://greasyfork.org/en/scripts/513435">AO3 FicTracker</a>',
                'FT_toread': '<a href="https://greasyfork.org/en/scripts/513435">AO3 FicTracker</a>',
                'FT_disliked': '<a href="https://greasyfork.org/en/scripts/513435">AO3 FicTracker</a>',
                'FT_userNotes': '<a href="https://greasyfork.org/en/scripts/513435">AO3 FicTracker</a>'
            };

            // Check for ScriptSync keys
            if (key.startsWith('SS_')) {
                return '<strong>ScriptSync</strong>';
            }
